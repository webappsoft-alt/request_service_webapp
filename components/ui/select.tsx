...
/** react-select-style trailing dots while options load. */
function SelectLoadingDots({ className }: { className?: string }) {
  return (
    <span
      role="status"
      aria-label="Loading"
    ...
    >
      <span className="size-[3px] animate-bounce rounded-full bg-muted-foreground/80 [animation-delay:-0.3s]" />
      <span className="size-[3px] animate-bounce rounded-full bg-muted-foreground/80 [animation-delay:-0.15s]" />
      <span className="size-[3px] animate-bounce rounded-full bg-muted-foreground/80" />
    </span>
  )
}
...
function SelectTrigger({
  ...
  size?: "sm" | "default"
  /** Shows trailing loading dots (react-select style) while options fetch. */
  loading?: boolean
}) {
  return (
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          // Fixed menu height with internal scroll (used app-wide).
          "relative z-50 max-h-60 min-w-36 origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-lg border border-border bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-200",
