...
function liveServicePath(
  provider: Provider,
  service: PublicProfessional["activeServices"][number],
) {
...
export function FixedServiceCatalog({
  ...
  provider: Provider;
  liveServices?: PublicProfessional["activeServices"];
}) {
      ...
      <section className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            These live services come from this provider&apos;s public catalog. Booking one opens the
            real checkout flow instead of the estimate pipeline.
          </p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          {liveServices.map((service) => {
            const photo = service.images[0];
            const bullets = service.commonServices.length
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-3 p-4">
                  <div>
                    <h3 className="font-semibold tracking-tight">{service.servicesName}</h3>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {provider.companyName} offers this as a priced public service.
                    </p>
                  </div>
                  <p className="text-lg font-semibold text-[#003F7D]">
                    ...
                    <span className="text-sm font-normal text-muted-foreground">
                      {serviceUnitLabel(
                        service.unit === "hour" || service.unit === "visit" ? service.unit : "job",
                      )}
                    </span>
                  </p>
                  {bullets.length ? (
