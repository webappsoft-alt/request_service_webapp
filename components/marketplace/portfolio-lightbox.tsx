...
export function PortfolioGallery({
  ...
  const scrollActiveThumb = useCallback((photoIndex: number) => {
    ...
    const thumbIndex = Math.max(0, photoIndex - 1);
    const item = scroller?.children[thumbIndex] as HTMLElement | undefined;
    ...
    <>
      <div className="overflow-hidden rounded-xl border border-black/15 bg-card">
        ...
        {rest.length ? (
          ...
          >
            {rest.map((photo, restIndex) => {
              const photoIndex = restIndex + 1;
              return (
              <li
