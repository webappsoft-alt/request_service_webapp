...
export function CreateCustomerNoteDialog({
  ...
  async function save() {
    ...
    // API requires non-empty `content`.
    ...
    try {
      if (note) {
        ...
        // Slice replaces the note by `id` from the Update API response — no GET.
...
export function CustomerNotesPanel({
  ...
  const dispatch = useAppDispatch();
  const slice = useAppSelector((state) => state.customerNotes);
  const {
    items,
    page,
    limit,
  ...
  useEffect(() => {
    ...
    void dispatch(fetchCustomerNotes({ entityId: customerId }));
  }, [dispatch, customerId, page, search, limit]);
  ...
  async function togglePin(row: CustomerNote) {
    ...
    // Instant UI update, then sync with the API in the background.
    dispatch(
      optimisticToggleNotePin({ id: row.id, isPinned: nextPinned }),
    );
    const result = await dispatch(
    ...
    // Revert on failure.
    dispatch(
      optimisticToggleNotePin({ id: row.id, isPinned: row.isPinned }),
    );
    toast.error(
      typeof result.payload === "string"
        ? result.payload
  ...
  function copyNote(row: CustomerNote) {
    const text = [noteDisplayTitle(row), row.content].filter(Boolean).join("\n\n");
  ...
  return (
    <div className="space-y-3">
      <PortalDataTable
        ...
        empty={empty ?? "No notes on this customer yet."}
